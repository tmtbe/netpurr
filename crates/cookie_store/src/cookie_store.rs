...
impl CookieStore {
    ...
    pub fn insert_raw_no_url_check(&mut self, cookie: &RawCookie<'_>) -> InsertResult {
        Cookie::try_from_raw_cookie_no_url_check(cookie)
            .and_then(|cookie| self.insert_no_url_check(cookie.into_owned()))
    }
    ...
    pub fn insert_no_url_check(&mut self, cookie: Cookie<'static>) -> InsertResult {
        if !cookie.is_expired() {
            Ok(
                if self
                    .cookies
